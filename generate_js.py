...
# 数据源：支持命令行传入路径，否则默认同目录
wb_path = sys.argv[1] if len(sys.argv) > 1 else os.path.join(BASE_DIR, '渠道数据.xlsx')
...
def _fmt_date(v):
    """Format date to M.D string like original dashboard (no leading zeros, no year)."""
    if v is None:
        return ''
    ...
    s = str(v).strip()
    # Handle Excel serial date numbers like "5.18", "05/18"
    m = re.match(r'^(\d{1,2})[./](\d{1,2})$', s)
    if m:
        return f'{int(m.group(1))}.{int(m.group(2))}'
    # Handle YYYY-MM-DD / YYYY/MM/DD string format
...
# ============================================================
# 3.5. CHANNEL_CITY — per-channel city breakdown from 渠道交付
# ============================================================
print('\n=== Generating CHANNEL_CITY ===')
ch_city_agg = defaultdict(lambda: defaultdict(lambda: {'orders': 0, 'arrival': 0, 'fail': 0}))
...
for r in range(2, ws_delivery.max_row + 1):
    ...
    d = ch_city_agg[ch_name][city]
    d['orders']  += _int(ws_delivery.cell(r, 8).value)
    d['arrival'] += _int(ws_delivery.cell(r, 11).value)
    d['fail']    += _int(ws_delivery.cell(r, 14).value)

# Build output: for each channel, list its cities sorted by orders desc
ch_city_out = {}
for ch_name, cities in ch_city_agg.items():
    city_list_out = []
    for city, d in sorted(cities.items(), key=lambda x: -x[1]['orders']):
        orders = d['orders']
        if orders < 5:  # skip trivial city-channel combos
            continue
        city_list_out.append({
            'city': city,
            'orders': orders,
            'arrivalRate': round(d['arrival'] / orders * 100, 2) if orders else 0.0,
            'cancelRate':  round(d['fail']    / orders * 100, 2) if orders else 0.0,
    ...
    if city_list_out:
        ch_city_out[ch_name] = city_list_out

print(f'  CHANNEL_CITY: {len(ch_city_out)} channels with city breakdown')
sample_ch = list(ch_city_out.keys())[:2]
for ch in sample_ch:
    sample_cities = ch_city_out[ch][:3]
    print(f'    {ch}: {[(c["city"], c["arrivalRate"]) for c in sample_cities]}')
lines.append('// =================== 每个渠道的城市表现 ===================')
lines.append('var CHANNEL_CITY = ' + json.dumps(ch_city_out, ensure_ascii=False, indent=2))
lines.append('')


# ============================================================
# 3.6. CHANNEL_PRODUCT — from 产品表现 sheet (per-channel product group breakdown)
# ============================================================
print('\n=== Generating CHANNEL_PRODUCT (from 产品表现 sheet) ===')
ws_product = wb['产品表现']

# Column mapping (even cols = product group name in row1, odd cols = 新单数(含转言))
product_cols = {}  # col_idx -> product_name
for c in range(6, ws_product.max_column + 1):
    h = ws_product.cell(1, c).value
    if h and str(h).strip():
        product_cols[c] = str(h).strip()

print(f'  Product groups: {list(product_cols.values())}')

# Aggregate: channel -> product -> orders
ch_product_agg = defaultdict(lambda: defaultdict(int))
for r in range(4, ws_product.max_row + 1):  # data starts from row4
    ch_name = ws_product.cell(r, 1).value
    if not ch_name or not str(ch_name).strip():
        continue
    ch_name = str(ch_name).strip()

    for col_idx, prod_name in product_cols.items():
        # The new单数 column is the even-numbered one (e.g., col6=空调组新单数)
        # The next column (col7) is 新单数(含转言)
        orders = _int(ws_product.cell(r, col_idx).value)
        if orders > 0:
            ch_product_agg[ch_name][prod_name] += orders

# Build output: for each channel, list products sorted by orders desc
ch_product_out = {}
for ch_name, products in ch_product_agg.items():
    total_prod_orders = sum(products.values())
    product_list_out = []
    for product, orders in sorted(products.items(), key=lambda x: -x[1]):
        pct = round(orders / total_prod_orders * 100, 1) if total_prod_orders > 0 else 0
        product_list_out.append({
            'product': product,
            'orders': orders,
            'pct': pct,
        })
    if product_list_out:
        ch_product_out[ch_name] = product_list_out

print(f'  CHANNEL_PRODUCT: {len(ch_product_out)} channels with product breakdown')
sample_ch = list(ch_product_out.keys())[:3]
for ch in sample_ch:
    prods = ch_product_out[ch][:5]
    print(f'    {ch}: {[(p["product"], p["orders"], f"{p["pct"]}%") for p in prods]}')
lines.append('// =================== 每个渠道的产品组表现 ===================')
lines.append('var CHANNEL_PRODUCT = ' + json.dumps(ch_product_out, ensure_ascii=False, indent=2))
lines.append('')
...
wp_labels  = sorted_wp_dates[:31]  # full month, up to 31 days
wp_overall = []
...
print(f'  CITY           : {len(city_list)} cities')
print(f'  CHANNEL_CITY   : {len(ch_city_out)} channels')
print(f'  CHANNEL_PRODUCT: {len(ch_product_out)} channels')
print(f'  INDUSTRY       : {len(industry_list)} industries')
